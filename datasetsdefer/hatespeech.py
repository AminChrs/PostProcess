...
import random
import logging
import datasetsdefer.predict as predict
import requests
...
class ModelPredictAAE:
    ...
    def load_model(self):
        ...
        self.vocab = [
            L.split("\t")[-1].strip() for L in open(self.vocabfile,
                                                    encoding="utf8")
        ]
    ...
    def infer_cvb0(self, invocab_tokens, alpha, numpasses):
        ...
        for i in range(0, doclen):
            ...
            Qs[i, :] /= Qs[i, :].sum()
        lik = Qs.copy()  # pertoken normalized but
        # proportionally the same for inference
    ...
    def predict_lang(self, tokens, alpha=1, numpasses=5, thresh1=1,
                     thresh2=0.2):
        ...
        # check that at least yy% of tokens are in vocabulary
            ...
            # posterior is probability for African-American,
            # Hispanic, Asian, and White (in that order)
...
class HateSpeech(BaseDataset):
    ...
    def __init__(
        ...
        include_demographics (bool): whether to include the demographics
        for each example, defined as either AA or not.
        if True, then the data loader will return a
        tuple of (data, label, expert_prediction, demographics)
        expert_type (str): either 'synthetic' which makes error depending on
        synth_exp_param for not AA or AA, or 'random_annotator'
        which defines human as random annotator
        synth_exp_param (list): list of length 2, first element is
        the probability of error for AA, second is for not AA
        test_split: percentage of test data
        val_split: percentage of data to be used for
        validation (from training set)
    ...
    def generate_data(self):
        ...
        if not os.path.exists(self.data_dir + "/hatespeech_labeled_data.csv"):
            ...
            r = requests.get(
                ...
                "-language/raw/master/data/labeled_data.csv",
                allow_redirects=True,
            )
            with open(self.data_dir + "/hatespeech_labeled_data.csv",
                      "wb") as f:
                f.write(r.content)
        ...
        if not os.path.exists(self.data_dir + "/model_count_table.txt"):
            ...
            r = requests.get(
                "https://github.com/slanglab/twitteraae/raw/master" +
                "/model/model_count_table.txt",
                allow_redirects=True,
            )
        ...
        self.model_aae = ModelPredictAAE(self.model_file_path,
                                         self.vocab_file_path)
        # predict demographics for the deata
        predict.load_model()

        def fuunc(x):
            ret = predict.predict(x.split())
            # if ret is None then print x
            if ret is None:
                return 2
            if ret[0] > 0.5:
                return 0
        ...
        if self.expert_type == "synthetic":
            for i in range(len(hatespeech_data)):
                if hatespeech_data.iloc[i]["demographics"] == 0:
                    correct_human = np.random.choice(
                        [0, 1], p=[1 - self.synth_exp_param[0],
                                   self.synth_exp_param[0]]
                    )

                else:
                    correct_human = np.random.choice(
                        [0, 1], p=[1 - self.synth_exp_param[1],
                                   self.synth_exp_param[1]]
                    )
                if correct_human:
            ...
            for i in range(len(hatespeech_data)):
                ...
                label_distribution = \
                    hatespeech_data.iloc[i]["label_distribution"]
        ...
        print("sum of train_d", sum(train_d == 1))
        ...
        if self.embed_texts:
            ...
            examples = [
                data.Example.fromlist([train_x[i], train_y[i],
                                       train_h[i]], fields)
    ...
    def model_setting(self, model_nn):
        ...
        # model = CNN_rej(INPUT_DIM, EMBEDDING_DIM, N_FILTERS, FILTER_SIZES,
        # 3, DROPOUT, PAD_IDX)
